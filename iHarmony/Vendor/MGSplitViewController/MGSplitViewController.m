...
- (CGSize)splitViewSizeForOrientation:(UIInterfaceOrientation)theOrientation
{
    ...
    // Alberto fix: since the split view is covered by the tab bar I reduce the height
    if (UIInterfaceOrientationIsLandscape(theOrientation)) {
        statusBarHeight = MAX((fullScreenRect.size.width - appFrame.size.width), (fullScreenRect.size.height - appFrame.size.height + 69));
    } else {
        statusBarHeight = MAX((fullScreenRect.size.width - appFrame.size.width), (fullScreenRect.size.height - appFrame.size.height + 49));
    }
...
- (void)reconfigureForMasterInPopover:(BOOL)inPopover
{
	...
	if (inPopover && !_hiddenPopoverController && !_barButtonItem) {
        ...
        // originally was passed a CGRectZero, but the log 'the rect passed in to this method must have non-zero width and height. This will be an exception in a future release.' convinced me to hack it
        if (self.view.window != nil) // added by Alberto De Bortoli to solve random crashes after in app long non-use
		[_hiddenPopoverController presentPopoverFromRect:CGRectMake(-10, -10, 1, 1) inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:NO];
...
- (IBAction)showMasterPopover:(id)sender
{
	...
	} else { // Alberto: fix missing dismiss on second tap 
